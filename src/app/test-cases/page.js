...
export default async function TestCasesList({ searchParams }) {
  ...
  return (
    <main className="p-8 w-full max-w-5xl mx-auto">
      ...
      {testCases.length === 0 ? (
        <p className="text-gray-500">{showingArchived ? 'No archived test cases.' : 'No test cases yet.'}</p>
      ) : (
        <ul className="space-y-3">
          {testCases.map((tc) => (
            <li key={tc.id} className="border rounded p-4 flex justify-between items-start gap-3">
              <div>
                <h2 className="font-semibold">
                  {tc.seq_number && <span className="text-gray-400 font-normal mr-2">{formatId('TC', tc.seq_number)}</span>}
                  {tc.title}
                </h2>
                <p className="text-sm text-gray-600 mt-1">{tc.expected_result}</p>
              </div>
              <form action={showingArchived ? restoreTestCase : archiveTestCase}>
                <input type="hidden" name="testCaseId" value={tc.id} />
                <button
                  type="submit"
                  className={`text-xs px-3 py-1 rounded whitespace-nowrap ${
                    showingArchived ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                  }`}
                >
